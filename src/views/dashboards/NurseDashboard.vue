  ...
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    <div class="space-y-6">
      ...
      <div class="bg-surface-dark p-6 rounded-lg shadow-lg text-center">
        <MdiIcon :path="mdiMotherNurse" size="48" class="mx-auto text-primary" />
        <h3 class="text-xl font-bold mt-4">Welcome, {{ authStore.user?.displayName || 'User' }}</h3>
        <p class="text-text-muted mt-2">
          {{ authStore.userClaims?.wardType ? `${authStore.userClaims.wardType} Ward` : 'General Ward' }}
        </p>
        <div class="mt-6 grid grid-cols-3 gap-4">
          <div class="text-center">
            <p class="text-3xl font-bold text-primary">{{ todayStats.vitalsChecked }}</p>
            <p class="text-sm text-text-muted">Vitals Checked</p>
          </div>
          <div class="text-center">
            <p class="text-3xl font-bold text-primary">{{ todayStats.notesAdded }}</p>
            <p class="text-sm text-text-muted">Notes Added</p>
          </div>
          <div class="text-center">
            <p class="text-3xl font-bold text-primary">{{ todayStats.patientsAssigned }}</p>
            <p class="text-sm text-text-muted">Assigned Patients</p>
          </div>
        </div>
      </div>
    </div>
